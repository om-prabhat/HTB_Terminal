...
case "$selected" in 
	"Show Active Machines")
		...
		active_options_selected=$(printf '%s\n' "${active_options[@]}"| rofi -config ~/.config/rofi/htb/themes/rasi/htb_theme.rasi -dmenu -p "Select an option (use ↑ ↓ arrows and Enter)")
	...
	"Show Running Machine Details")
		...
		if [[ "$running_machine_id" == "null" && -n "$running_machine_id" ]];then
			echo "OK" | rofi -config ~/.config/rofi/htb/themes/rasi/dialog.rasi -dmenu -p "No active Machine!" > /dev/null
			exit 0
		fi
